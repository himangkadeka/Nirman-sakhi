...
defined('PUBLIC_KEY') or die('PUBLIC_KEY not defined in Environment. Please include bridge config file(s)');
defined('PRIVATE_KEY') or die('PRIVATE_KEY not defined in Environment. Please include bridge config file(s)');

class BridgePG {

    public function __construct() {
        $this->vk = base64_decode(PRIVATE_KEY);
        $this->uk = base64_decode(PUBLIC_KEY);
    }
...
//    }
    ...
    public function decryptTestCsc(Request $request)
    {
        $encrypted = $request->input('encrypted'); // the encrypted string from Postman

        // Call your existing decrypt_wallet_message method
        $output = '';
        $result = $this->decrypt_wallet_message($encrypted, $output);
