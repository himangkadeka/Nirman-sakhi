...
require_once(app_path('Libraries/CSCPay/BridgePG.php'));
require_once(app_path('Libraries/CSCPay/bridge_config.php'));
...
//			'merchant_txn'           => 'P121' . time() . rand(10,99),//'2016-06-21 18:11:58',
            'merchant_txn' => 'P121' . (time() + 300) . rand(10,99),
//			'merchant_txn_date_time' => date('Y-m-d H:i:s'),//'2016-06-21 18:11:58',
            'merchant_txn_date_time' => date('Y-m-d H:i:s', time() + 300),
			'product_id'             => '4938787463',
			'product_name'           => 'Forest Department Government of Chattisgarh',
			'txn_amount'             => '100',
			'amount_parameter'       => 'NA',
			'txn_mode'               => 'D',
			'txn_type'               => 'D',
    ...
    public function get_fraction($ddhhmm = ""){
        ...
        if(!$ddhhmm)
            $ddhhmm = date($time_format, time() + 300);

        $frac = $this->large_op1($ddhhmm, $algo_num );
        $frac = $this->large_op2($frac, "" . (1000 - $algo_num) );
        return $frac;
    }
	public function reverse_log($tid,$merchant_date){
		...
		$data = array(
			...
			'merchant_txn'        => $tid,
			'merchant_txn_datetime'      => date("Y-m-d H:i:s", strtotime($merchant_date)),
			
			
		);
		
		$result = $this->_call_bridge_api('transaction/reverse', $data);
...
//		return $data;
	...
	function recon_log($tid, $csc_txn, $cscuser_id, $product_id, $txn_amount, $merchant_date, $merchant_txn_status, $merchant_reciept){
		if(!isset($this->merchant_id)){
			//
			exit("Merchant ID not set. Please call set_mid first.");
		}
...
//        return $message_cipher;
		...
		// $url='https://bridgeuat.csccloud.in/cscbridge/v2/'.trim($method, '/') . '/format/json';
        //production url
		$url='https://bridge.csccloud.in/v2/'.trim($method, '/') . '/format/json';
        //staging url
//		$url='https://bridgeuat.csccloud.in/cscbridge/v2/'.trim($method, '/') . '/format/json';
