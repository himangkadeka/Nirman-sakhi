...
@section('content')
    <div class="nirman-sahi-dashboard-container mx-5">
        ...
        <div id="dlc-content" class="tab-content active">
            ...
            <h3 style="margin-top:20px;">Applications Forwarded to DLC</h3>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function performForwardAction(applicationId, buttonElement) {
            ...
            fetch("{{ route('office.dlc-dashboard.forwardToLc') }}", { // Make sure this route name is correct!
                    method: 'POST',
                    headers: {
        ...
        applicationsTable.addEventListener('click', async function(event) {
            ...
            if (button.classList.contains('preview-button')) {
                ...
                modalBody.innerHTML = `<div class="text-center p-5"><div class="spinner-border"></div><p>Loading...</p></div>`;
                previewModal.show();
            ...
            if (button.classList.contains('view-log-btn')) {
                ...
                modalBody.innerHTML = `<div class="text-center p-5"><div class="spinner-border"></div><p>Fetching history...</p></div>`;
                logModal.show();
                 ...
                 fetch(`/office/dashboard/applications/${applicationId}/log`)
                    ...
                    .catch(error => {
                        ...
                        modalBody.innerHTML = `<div class="alert alert-danger">Failed to load logs.</div>`;
                    });
            }

            // --- Forward Logic (using async/await) ---
            if (button.classList.contains('forward-single-btn')) {
                const confirmed = await showConfirmation('Are you sure you want to forward this application to the Labour Commissioner?');
                if (confirmed) {
