...
@section('content')
        ...
        <div id="accounts" class="tab-content active">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">PPA Signing Queue</h4>
                </div>
                <div class="card-body">
                    <p>The following batches are ready for signing. Download the PPA, get it signed, and upload the signed
                        copy to proceed.</p>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse($pendingSignatureBatches as $batch)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <a href="{{ route('office.accounts.ppa.download', ['batch_id' => $batch->batch_id]) }}"
                                                    class="btn btn-sm btn-info">
                                                    <i class="fas fa-download"></i> Download PPA
                                                </a>
                                                <button class="btn btn-sm btn-success upload-signed-btn"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center p-3">There are no batches awaiting signature.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            <table id="applicationsTable">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center;">No applications found in the accounts queue.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    {{-- NEW: Modal for uploading the signed PPA --}}
    <div class="modal fade" id="uploadSignedPpaModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Upload Signed PPA</h5>
                    <button type="button" class="btn-close" data-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('office.accounts.ppa.uploadSigned') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">Batch ID</label>
                            <input type="text" class="form-control" id="modal_signed_batch_id" name="batch_id" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="signed_ppa_file" class="form-label">Signed PPA File (PDF, JPG, PNG)</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const uploadModalEl = document.getElementById('uploadSignedPpaModal');
            const uploadModal = new bootstrap.Modal(uploadModalEl);

            // Use event delegation for the upload buttons
            document.body.addEventListener('click', function(event) {
                const uploadButton = event.target.closest('.upload-signed-btn');
                if (uploadButton) {
                    const batchId = uploadButton.dataset.batchId;
                    // Set the batch ID in the modal's readonly input field
                    uploadModalEl.querySelector('#modal_signed_batch_id').value = batchId;
                    uploadModal.show();
                }
            });
        });
