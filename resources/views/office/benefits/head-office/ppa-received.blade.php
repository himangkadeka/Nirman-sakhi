...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group">
                                                ...
                                                <button class="btn btn-sm btn-success forward-to-hda-btn"
                                                    data-batch-id="{{ $batch->batch_id }}">
                                                    <i class="fas fa-share-square"></i> Forward to HDA
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('office.lc-lm-dashboard.ppa.uploadFinalSignature') }}" method="POST"
    ...
    <div class="modal fade" id="forwardToHdaModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Forward Batch for Disbursement</h5>
                    <button type="button" class="btn-close" data-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form id="forwardToHdaForm">
                        <div class="mb-3">
                            <label class="form-label">Batch ID</label>
                            <input type="text" class="form-control" id="modal_hda_batch_id" name="batch_id" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="hda_user_select" class="form-label">Select Head Dealing Assistant (HDA)</label>
                            <select class="form-control" id="hda_user_select" name="hda_user_id" required>
                                <option value="" selected disabled>- Select an Officer -</option>
                                @foreach ($headDealingAssistants as $hda)
                                    <option value="{{ $hda->id }}">{{ $hda->firstname }} {{ $hda->lastname }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="hda_forward_comment" class="form-label">Comment (Optional)</label>
                            <textarea class="form-control" id="hda_forward_comment" name="comment" rows="3"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="submitForwardToHdaBtn">Confirm & Forward</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- NEW: Get HDA Modal elements ---
            const hdaModalEl = document.getElementById('forwardToHdaModal');
            const hdaModal = new bootstrap.Modal(hdaModalEl);
            const submitHdaBtn = document.getElementById('submitForwardToHdaBtn');

            // Use event delegation for all buttons
            document.body.addEventListener('click', function(event) {

                // --- Handler for the "Forward to HDA" button ---
                const forwardHdaButton = event.target.closest('.forward-to-hda-btn');
                if (forwardHdaButton) {
                    const batchId = forwardHdaButton.dataset.batchId;
                    // Pre-fill the batch ID in the modal
                    hdaModalEl.querySelector('#modal_hda_batch_id').value = batchId;
                    hdaModal.show();
                }
            });

            // --- Listener for the SUBMIT button inside the HDA modal ---
            submitHdaBtn.addEventListener('click', function() {
                const form = document.getElementById('forwardToHdaForm');
                const batchId = form.querySelector('#modal_hda_batch_id').value;
                const hdaUserId = form.querySelector('#hda_user_select').value;
                const comment = form.querySelector('#hda_forward_comment').value;

                // Basic validation
                if (!hdaUserId) {
                    alert('Please select a Head Dealing Assistant.');
                    return;
                }
                ...
                fetch("{{ route('office.head-office.hda.disbursement') }}", {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            hdaModal.hide();
                            Swal.fire("Success!",data.message,'success'); // Or use Swal.fire
