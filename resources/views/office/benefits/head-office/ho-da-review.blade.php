...
@section('content')
    <div class="nirman-sahi-dashboard-container mx-5">
        ...
        <div id="daReview" class="tab-content active">
            <h2>HDO Reviewed / DA Approved Applications</h2>

            {{-- This button will trigger our new JavaScript --}}
            <div class="mb-3">
                <button type="button" class="btn btn-primary" id="forwardToAccountsBtn">
                    <i class="fas fa-paper-plane"></i> Forward Selected to Accounts
                </button>
            </div>

            <table id="daReviewTable"> {{-- Give the table a unique ID --}}
                <thead>
                    <tr>
                        <th><input type="checkbox" id="selectAllDaReview"></th>
                        <th>App ID</th>
                        <th>Worker ID</th>
                ...
                <tbody>
                    @forelse ($hdoReviewedApplications as $app)
                        <tr>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center;">No applications are ready to be forwarded.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            <div class="mt-4">
                {{ $hdoReviewedApplications->links() }}
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // --- Get references to elements for this specific tab ---
            const forwardBtn = document.getElementById('forwardToAccountsBtn');
            const selectAllCheckbox = document.getElementById('selectAllDaReview');
            const rowCheckboxes = document.querySelectorAll('#daReviewTable .app-checkbox');
            ...
            rowCheckboxes.forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    ...
                    } else if (document.querySelectorAll('#daReviewTable .app-checkbox:checked')
            ...
            // --- Forward to Accounts Button Logic ---
            if (forwardBtn) {
                forwardBtn.addEventListener('click', function() {
                    // 1. Collect IDs of all checked applications
                    const selectedIds = Array.from(document.querySelectorAll(
                            '#daReviewTable .app-checkbox:checked'))
                        .map(cb => cb.dataset.id);

                    // 2. Validate that at least one is selected
                    if (selectedIds.length === 0) {
                        alert('Please select at least one application to forward.');
                        return;
                    }

                    // 3. Confirm the action with the user
                    if (!confirm(
                            `Are you sure you want to forward ${selectedIds.length} application(s) to Accounts?`
                        )) {
                        return;
                    }
                    ...
                    fetch("{{ route('office.head-office.applications.forwardToAccounts') }}", {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire('Success!', data.message,
                                    'success'); // Or use a nicer alert library like SweetAlert
                        ...
                        .catch(error => {
                            ...
                            Swal.fire('Error!', 'A network or server error occurred. Please try again.',
                                'error');
                        })
                        .finally(() => {
