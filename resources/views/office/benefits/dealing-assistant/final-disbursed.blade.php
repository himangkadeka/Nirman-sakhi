...
@section('content')
                                ...
                                @forelse($pendingSignatureBatches as $batch)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-success mark-disbursed-btn"
                                                data-batch-id="{{ $batch->batch_id }}">
                                                <i class="fas fa-money-check-alt"></i> Mark as Disbursed
                                            </button>
                                        </td>
                                    </tr>
                                @empty
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Mark Batch as Disbursed</h5>
                ...
                <div class="modal-body">
                    <form id="disbursementForm">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="modal_disburse_batch_id" name="batch_id"
                                readonly>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="disburse_comment" class="form-label">Comment (Optional)</label>
                            <textarea class="form-control" id="disburse_comment" name="comment" rows="3"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="submitDisbursementBtn">Confirm Disbursement</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Get modal elements
            const disbursementModalEl = document.getElementById('disbursementModal');
            const disbursementModal = new bootstrap.Modal(disbursementModalEl);
            const submitDisbursementBtn = document.getElementById('submitDisbursementBtn');

            // Listener for "Mark as Disbursed" buttons
            document.body.addEventListener('click', function(event) {
                const disburseButton = event.target.closest('.mark-disbursed-btn');
                if (disburseButton) {
                    const batchId = disburseButton.dataset.batchId;
                    disbursementModalEl.querySelector('#modal_disburse_batch_id').value = batchId;
                    disbursementModal.show();
                }
            });

            // Listener for the SUBMIT button inside the modal
            submitDisbursementBtn.addEventListener('click', function() {
                const form = document.getElementById('disbursementForm');
                const batchId = form.querySelector('#modal_disburse_batch_id').value;
                const transactionId = form.querySelector('#transaction_id').value;
                const comment = form.querySelector('#disburse_comment').value;
                ...
                fetch("{{ route('office.head-office-da.markAsDisbursed') }}", {
                        method: 'POST',
                        headers: {
                    ...
                    .finally(() => {
                        this.disabled = false;
                        this.innerHTML = `Confirm Disbursement`;
                    });
            });
        });
    </script>
