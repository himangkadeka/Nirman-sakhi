...
@section('title', 'Admin | Payment Reports CSC')
@section('breadcrumb_item_1', 'MIS Data')
@section('breadcrumb_item_2', 'Payment Reports CSC')
...
@section('style')
    <style>
        * { box-sizing: border-box; }
        ...
        .summary-card.apps  { border-left-color: #534AB7; }
        ...
        .search-group input.form-control {
            ...
            border-radius: 3px; border: 0.5px solid #ced4da; width: 260px;
        }
        .btn-search {
        ...
        .tbl-gov .amount { font-weight: 500; font-variant-numeric: tabular-nums; text-align: right; }
        ...
        .btn-act {
            ...
            cursor: pointer; display: inline-block; margin-right: 2px; line-height: 1.5;
        }
        ...
        .modal-header   { background: #f1f3f5; border-bottom: 0.5px solid #dee2e6; padding: 8px 12px; }
        ...
        .modal-footer   { padding: 8px 12px; border-top: 0.5px solid #e9ecef; justify-content: flex-end; gap: 6px; display: flex; }
        .btn-modal-primary {
    ...
    <div class="container-fluid">
        ...
        <div class="page-header">
            <div>
                ...
                <div class="page-title">Payment Reports — CSC</div>
            </div>
        ...
        <div class="summary-row">
            <div class="summary-card apps">
                <div class="card-label">Applications Received</div>
                <div class="card-value">{{ number_format($applications) }}</div>
                <div class="card-sub">Via CSC channel</div>
            </div>
            <div class="summary-card total">
                <div class="card-label">Total Payment Received</div>
                <div class="card-value">₹ {{ number_format($total, 2) }}</div>
                <div class="card-sub">All payment types</div>
            </div>
            <div class="summary-card reg">
                <div class="card-label">Registration Payments</div>
                <div class="card-value">₹ {{ number_format($totalRegistration, 2) }}</div>
                <div class="card-sub">Registration fee collected</div>
            </div>
            <div class="summary-card sub">
                <div class="card-label">Subscription Payments</div>
                <div class="card-value">₹ {{ number_format($totalSubscription, 2) }}</div>
                <div class="card-sub">Renewal subscriptions</div>
            </div>
        </div>

        {{-- Toolbar --}}
        <form method="GET" action="{{ route('admin.csc.csc-data') }}">
            <div class="toolbar">
                ...
                <div class="search-group">
                    <input type="text" name="search" class="form-control"
                           placeholder="Search by Worker ID, CSC ID, TXN ID"
            ...
            <table class="tbl-gov">
                ...
                @foreach ($payments as $payment)
                    <tr>
                        ...
                        <td>{{ $payment->csc_id }}</td>
                        <td>{{ $payment->csc_txn_id }}</td>
                        <td>{{ $payment->merchant_id }}</td>
                        <td>{{ $payment->merchant_txn }}</td>
                        <td>{{ $payment->merchant_receipt_no }}</td>
                        <td>
                        ...
                        <td>{{ \Carbon\Carbon::parse($payment->merchant_txn_datetime)->format('d-m-Y') }}</td>
                        <td>
