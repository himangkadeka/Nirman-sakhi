...
@section('title', ' | Renewal|Certificate Details')
                                        ...
                                        <div class="table-container">
                                            <table class="table" id="user_table">
                                                ...
                                                <tr>
                                                    ...
                                                    <th scope="col" class="bold">Type of Issuer / ইছ্যুকাৰীৰ
                                                    ...
                                                    <th scope="col" class="bold">Type Of Construction
                                                        Work&nbsp;|&nbsp;নিৰ্মাণ কামৰ ধৰণ<span
                                                            class="text-danger">*</span></th>
...
{{--                                                    <th scope="col" class="bold">Action</th>--}}
                                                ...
                                                <tbody>
                                                @foreach ($mwc as $key => $certificate)
                                                    <tr>
                                                        ...
                                                        {{-- <td class="fixed-width"><input type="text"
                                                                ...
                                                                value="{{ $certificate->employer_contact_name }}"
                                                                class="form-control"
                                                                {{ $certificate->is_same == 1 ? 'readonly' : '' }}
                                                                id="employer_contact_name_{{ $loop->index + 1 }}"
                                                                maxlength="10" />
                                                            <span class="text-danger success"
                                                        ...
                                                        <td class="fixed"><input type="text" id="dateCount"
                                                                                 class="form-control date_count" name="date_count[]" />
                                                        </td>
                                                        ...
                                                        {{-- <td class="fixed-width">
                                                            <input type="file" name="certificate_proof[]"
                                                                class="form-control fixed-width" data-id="1">
                                                        ...
                                                        {{-- <td class="fixed-width">
                                                            <a class="btn  btn-sm" target="_blank"
                                                                href="{{ route('get-cert-proof', ['id' => $certificate->id]) }}">
                                                    ...
                                                    <ul class="list-group">
                                                        @foreach ($ndc as $certificate)
                                                            <li class="list-group-item">
                                                                ...
                                                                <a href="{{ $certificate->ninety_days_certificate }}"><button type="button" class="btn btn-sm remove-file"><i class="fa fa-eye text-primary"></i></button></a>
                                                                <button type="button" class="btn btn-sm remove-file"><i class="fa fa-trash text-danger"></i></button>
                                                            </li>
...
@section('footer')
    ...
    <script>
        $(document).ready(function() {
            ...
            <td class="dropdown">
                ...
                html += '<td><input type="text" name="contact_issuing_person[' + count +
            ...
            $('#employer').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('save-certificate-details') }}',
                    method: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
    ...
    <script>
        let fileIndex = 1; // Initialize the index for file inputs

        document.getElementById('add-more').addEventListener('click', function() {
            // Create a new div for the additional file input
            const fileInputWrapper = document.createElement('div');
            fileInputWrapper.classList.add('file-input-wrapper', 'mt-2'); // Add margin for spacing

            // Add the new input element with a remove button
            fileInputWrapper.innerHTML = `
            <div class = d-flex>
                <input type="file" name="certificate_proof[]" id="certificate_proof_${fileIndex}" class="form-control fixed-width" data-id="${fileIndex}">
                <span class="text-danger success" id="certificate_proof_${fileIndex}_error"></span>
                <button type="button" class="btn btn-sm ml-2 remove-file"><i class="fa fa-trash text-danger"></i></button>
            </div>
            `;

            // Append the new file input to the file-inputs div
            document.getElementById('file-inputs').appendChild(fileInputWrapper);

            // Increment the file input index for the next input
            fileIndex++;
        });
        ...
        document.getElementById('file-inputs').addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('remove-file')) {
                ...
                wrapper.remove(); // Remove the file input wrapper
            }
        });
    </script>
